 ...
 * \author Yarin Andrey <a.yarin@is-telecom.ru>
 * \brief Lua PBX Switch with LuaJIT support
 *
 * \ingroup PBX modules
 */
...
#include "pbx_luajit.h"

static char *config = "extensions.lua";
static char *registrar = "pbx_luajit";
 ...
 */
EXPORT_DEF void lua_state_destroy(void *data)
{
    if (data)
#ifdef DEBUG
 ...
 */
EXPORT_DEF void lua_datastore_fixup(void *data, struct ast_channel *old_chan, struct ast_channel *new_chan)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_datastore_fixup\n");
#endif
    lua_State *L = data;
 ...
 * \endcode
 */
EXPORT_DEF int lua_pbx_findapp(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_pbx_findapp\n");
#endif
 ...
 * \endcode
 */
EXPORT_DEF int lua_pbx_exec(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_pbx_exec\n");
#endif
 ...
 * \endcode
 */
EXPORT_DEF int lua_get_variable_value(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_get_variable_value\n");
#endif
    struct ast_channel *chan;
 ...
 * \endcode
 */
EXPORT_DEF int lua_set_variable_value(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_set_variable_value\n");
#endif
 ...
 */
EXPORT_DEF void lua_update_registry(lua_State *L, const char *context, const char *exten, int priority)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_update_registry\n");
#endif
    lua_pushstring(L, context);
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF void lua_push_variable_table(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_push_variable_table\n");
#endif
    lua_newtable(L);
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF void lua_create_app_table(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_create_app_table\n");
#endif
    lua_newtable(L);
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF void lua_create_channel_table(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_create_channel_table\n");
#endif
    lua_newtable(L);
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF void lua_create_variable_metatable(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_create_variable_metatable\n");
#endif
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF void lua_create_application_metatable(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_create_application_metatable\n");
#endif
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF void lua_create_autoservice_functions(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_create_autoservice_functions\n");
#endif
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF void lua_create_hangup_function(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_create_hangup_function\n");
#endif
 ...
 * \endcode
 */
EXPORT_DEF int lua_get_variable(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_get_variable\n");
#endif
    struct ast_channel *chan;
 ...
 * \endcode
 */
EXPORT_DEF int lua_set_variable(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_set_variable\n");
#endif
    struct ast_channel *chan;
    int autoservice;
    const char *name = luaL_checkstring(L, 2);
 ...
 */
EXPORT_DEF void lua_concat_args(lua_State *L, int start, int nargs)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_concat_args\n");
#endif
    int concat = 0;
    int i = start + 1;
 ...
 */
EXPORT_DEF int lua_func_read(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_func_read\n");
#endif
 ...
 */
EXPORT_DEF int lua_autoservice_start(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_autoservice_start\n");
#endif
    struct ast_channel *chan;
 ...
 */
EXPORT_DEF int lua_autoservice_stop(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_autoservice_stop\n");
#endif
    struct ast_channel *chan;
 ...
 */
EXPORT_DEF int lua_autoservice_status(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_autoservice_status\n");
#endif
    lua_getfield(L, LUA_REGISTRYINDEX, "autoservice");
    return 1;
}
 ...
 */
EXPORT_DEF int lua_check_hangup(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_check_hangup\n");
#endif
    struct ast_channel *chan;
    lua_getfield(L, LUA_REGISTRYINDEX, "channel");
    chan = lua_touserdata(L, -1);
 ...
 * \param L the lua_State to use
 */
EXPORT_DEF int lua_error_function(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_error_function\n");
#endif
    int message_index;
 ...
 * \retval 1 failure
 */
EXPORT_DEF int lua_sort_extensions(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_sort_extensions()\n");
#endif
...
#ifdef DEBUG
            ast_debug(2, "lua_sort_extensions() context:%i exten:%i\n", context, exten);
#endif
 ...
 * \retval 1 failure
 */
EXPORT_DEF int lua_register_switches(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_register_switches\n");
#endif
    int extensions;
 ...
 * \retval 1 failure
 */
EXPORT_DEF int lua_register_hints(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_register_hints\n");
#endif
    int hints;
    struct ast_context *con = NULL;
 ...
 */
EXPORT_DEF int lua_extension_cmp(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_extension_cmp()\n");
#endif
    ...
    if (ast_extension_cmp(a, b) == -1) {
        lua_pushboolean(L, 1);
#ifdef DEBUG
        ast_debug(2, "Call ast_extension_cmp == -1 for a:%s b:%s\n", a, b);
#endif
    } else {
        lua_pushboolean(L, 0);
#ifdef DEBUG
        ast_debug(2, "Call ast_extension_cmp else for a:%s b:%s\n", a, b);
#endif
    }
 ...
 */
EXPORT_DEF char *lua_read_extensions_file(lua_State *L, long *size)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_read_extensions_file\n");
#endif
    FILE *f;
    int error_func;
    char *data;
 ...
 * \retval 1 failure
 */
EXPORT_DEF int lua_load_extensions(lua_State *L, struct ast_channel *chan)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_load_extensions\n");
#endif
 ...
 */
EXPORT_DEF int lua_reload_extensions(lua_State *L)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_reload_extensions\n");
#endif
    long size = 0;
    char *data = NULL;
 ...
 */
EXPORT_DEF void lua_free_extensions()
{
#ifdef DEBUG
    ast_debug(2, "Call lua_free_extensions\n");
#endif
    ast_mutex_lock(&config_file_lock);
    config_file_size = 0;
    ast_free(config_file_data);
 ...
 * \return a lua_State
 */
EXPORT_DEF lua_State *lua_get_state(struct ast_channel *chan)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_get_state %p\n", chan);
#endif
    ...
    if (!chan) {
        L = luaL_newstate();
#ifdef DEBUG
        ast_debug(2, "Call lua_get_state: lua state: %p\n", L);
#endif
...
EXPORT_DEF int exists(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
#ifdef DEBUG
    ast_debug(2, "Call exists() context:%s exten:%s callerid:%s\n", context, exten, callerid);
#endif
    int res;
    lua_State *L;
...
EXPORT_DEF int canmatch(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
#ifdef DEBUG
    ast_debug(2, "Call canmatch\n");
#endif
    int res;
    lua_State *L;
    struct ast_module_user *u = ast_module_user_add(chan);
...
EXPORT_DEF int matchmore(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
#ifdef DEBUG
    ast_debug(2, "Call matchmore\n");
#endif
    int res;
    lua_State *L;
    struct ast_module_user *u = ast_module_user_add(chan);
...
EXPORT_DEF int exec(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
#ifdef DEBUG
    ast_debug(2, "Call exec()\n");
#endif
    int res, error_func;
    lua_State *L;
    struct ast_module_user *u = ast_module_user_add(chan);
...
#ifdef DEBUG
        ast_debug(2, "Call exec() dont find exten:%s in context:%s \n", exten, context);
#endif
        lua_pop(L, 1); /* pop the debug function */
        ast_log(LOG_ERROR, "Could not find extension %s in context %s by pbx_luajit\n", exten, context);
 ...
 */
EXPORT_DEF int lua_find_extension(lua_State *L, const char *context, const char *exten, int priority, ast_switch_f *func, int push_func)
{
#ifdef DEBUG
    ast_debug(2, "Call lua_find_extension()\n");
#endif
...
#ifdef DEBUG
    ast_debug(2, "lua_find_extension() context_table:%i\n", context_table);
#endif
...
#ifdef DEBUG
    ast_debug(2, "lua_find_extension() context_order_table:%i\n", context_order_table);
#endif
...
#ifdef DEBUG
        ast_debug(2, "lua_find_extension() e:%s\n", e);
#endif

        if (!(e = lua_tostring(L, e_index_copy))) {
#ifdef DEBUG
            ast_debug(2, "lua_find_extension() e:%s e_index_copy:%i\n", e, e_index_copy);
#endif
            lua_pop(L, 2);
            continue;
        }

        /* make sure this is not the 'include' extension or s extension*/
//        if (!strcasecmp(e, "include") || !strcasecmp(e, "s"))
...
#ifdef DEBUG
            ast_debug(2, "lua_find_extension() push_func context_table:%i\n", push_func);
#endif
    ...
    /*try to load s exten for this context */
    /*
        lua_getfield(L, context_table, "s");
        if (lua_isnil(L, -1))
        {
            ast_log(LOG_VERBOSE, "Get extension s:\n");
            lua_pop(L, -3);
            return 0;
        }
    */
...
static int load_or_reload_lua_stuff(void)
{
    ...
    if (!res) {
        ast_log(LOG_NOTICE, "LuaJIT PBX Switch loaded.\n");
    }
...
static int unload_module(void)
{
    ...
    lua_free_extensions();
    ast_log(LOG_NOTICE, "LuaJIT PBX Switch unloaded.\n");
    return 0;
}
...
static int load_module(void)
{
    ...
    if (ast_register_switch(&lua_switch)) {
        ast_log(LOG_ERROR, "Unable to register LUAJIT PBX switch\n");
        return AST_MODULE_LOAD_DECLINE;
    }
